...
(function() {
    ...
    function initDashboardCards() {
        ...
        `;
        document.querySelector('.dashboard-grid').appendChild(dropZoneIndicator);
    }
    ...
    function onMove(e) {
        ...
        // Show drop zone indicator at potential drop target
    ...
    function showDropZone(cursorX, cursorY) {
        ...
        const cards = grid.querySelectorAll('.dash-card[data-card-id]');
    ...
    function onEnd() {
        ...
        if (match) {
            ...
            // Only swap if moved more than 30px
            if (Math.abs(deltaX) > 30 || Math.abs(deltaY) > 30) {
                // Get the target card to swap with
                const targetId = dropZoneIndicator?.dataset?.targetId;
                if (targetId && targetId !== card.dataset.cardId) {
                    swapCards(card.dataset.cardId, targetId);
                }
            }
    ...
    function swapCards(cardIdA, cardIdB) {
        const cardA = document.querySelector(`.dash-card[data-card-id="${cardIdA}"]`);
        const cardB = document.querySelector(`.dash-card[data-card-id="${cardIdB}"]`);
        if (!cardA || !cardB) return;

        const grid = document.querySelector('.dashboard-grid');
        if (!grid) return;

        // Swap grid-area (keep them in grid flow)
        const areaA = cardA.style.gridArea || '';
        const areaB = cardB.style.gridArea || '';

        // Also swap any inline grid-column styles
        const colA = cardA.style.gridColumn || '';
        const colB = cardB.style.gridColumn || '';

        // Swap
        cardA.style.gridArea = areaB;
        cardB.style.gridArea = areaA;
        cardA.style.gridColumn = colB;
        cardB.style.gridColumn = colA;

        // Save layout
        saveLayout();
        showToast(`📦 ${cardIdA} ↔ ${cardIdB} swapped`, 'info');
    }
    ...
    function loadLayout() {
        try {
            ...
            Object.keys(layout).forEach(cardId => {
                ...
                if (card && layout[cardId]) {
                    if (layout[cardId].gridArea) {
                        card.style.gridArea = layout[cardId].gridArea;
                    }
                    if (layout[cardId].gridColumn) {
                        card.style.gridColumn = layout[cardId].gridColumn;
                    }
                    ...
                    // Clean up any absolute positioning from old version
                    card.style.position = '';
                    card.style.left = '';
                    card.style.top = '';
                    card.style.width = '';
